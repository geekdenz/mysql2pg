...
function run_stmt(mysqli $db, string $label, string $sql, ?string $param = null): void
{
    ...
    while ($field = $meta->fetch_field()) {
        echo "FIELD_{$label} name={$field->name} type={$field->type}\n";
    }
...
run_stmt($db, 'TWO', "SHOW VARIABLES LIKE 'character_set_database'");
run_stmt($db, 'THREE', "SHOW CHARACTER SET WHERE `Charset` = ?", 'utf8mb4');
