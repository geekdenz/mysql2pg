...
pub struct ServerConfig {
    ...
    #[serde(default = "default_mysql_bind_addr")]
...
impl Default for ServerConfig {
    fn default() -> Self {
        Self {
            ...
            mysql_bind_addr: default_mysql_bind_addr(),
        }
    }
}
...
impl AppConfig {
    ...
    pub fn from_env() -> Result<Self, MiddlewareError> {
        ...
        let bind_addr = env::var("MW_SERVER_BIND_ADDR").unwrap_or_else(|_| default_http_bind_addr());
        let mysql_bind_addr = env::var("MW_MYSQL_BIND_ADDR").unwrap_or_else(|_| default_mysql_bind_addr());

        Ok(Self {
            server: ServerConfig { bind_addr, mysql_bind_addr },
            postgres: PostgresConfig { driver, connection_string },
            translator: TranslatorConfig {
                rewrite_limit_comma: env_bool("MW_TRANSLATOR_REWRITE_LIMIT_COMMA", default_rewrite_limit_comma()),
                normalize_mysql_backticks: env_bool("MW_TRANSLATOR_NORMALIZE_MYSQL_BACKTICKS", default_mysql_backticks()),
                normalize_boolean_literals: env_bool("MW_TRANSLATOR_NORMALIZE_BOOLEAN_LITERALS", default_boolean_literals()),
    ...
    pub fn apply_env_overrides(&mut self) -> Result<(), MiddlewareError> {
        ...
        if let Ok(mysql_bind_addr) = env::var("MW_MYSQL_BIND_ADDR") {
            self.server.mysql_bind_addr = mysql_bind_addr;
        }
        ...
        self.translator.rewrite_limit_comma = env_bool("MW_TRANSLATOR_REWRITE_LIMIT_COMMA", self.translator.rewrite_limit_comma);
        self.translator.normalize_mysql_backticks = env_bool("MW_TRANSLATOR_NORMALIZE_MYSQL_BACKTICKS", self.translator.normalize_mysql_backticks);
        self.translator.normalize_boolean_literals = env_bool("MW_TRANSLATOR_NORMALIZE_BOOLEAN_LITERALS", self.translator.normalize_boolean_literals);
        self.translator.rewrite_mysql_functions = env_bool("MW_TRANSLATOR_REWRITE_MYSQL_FUNCTIONS", self.translator.rewrite_mysql_functions);
        self.translator.rewrite_json_operators = env_bool("MW_TRANSLATOR_REWRITE_JSON_OPERATORS", self.translator.rewrite_json_operators);
        self.translator.strip_mysql_table_options = env_bool("MW_TRANSLATOR_STRIP_MYSQL_TABLE_OPTIONS", self.translator.strip_mysql_table_options);

        if self.postgres.connection_string.trim().is_empty() {
            return Err(MiddlewareError::Config("PostgreSQL connection string cannot be empty".to_string()));
        }
